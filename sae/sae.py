...
class SAEConfig:
    def __init__(
        ...
        n_buffers: int
        | None = None,  # Number of buffers to train for, defaults to the full dataset
    ...
    def name(self):
        ret = (
            f"{self.point.layer}-{self.point.name}-x{self.expansion}-k{self.k}".replace(
                "_", "-"
            )
        )
...
class SAE(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.w_dec.weight.data /= torch.norm(
            self.w_dec.weight.data, dim=-2, keepdim=True
        )

        if config.bilinear_encoder:
            self.w_enc = Bilinear(
                self.d_model, self.d_features, bias=config.encoder_bias
            )
        else:
    ...
    def from_pretrained(repo_id, point, expansion, k):
        ...
        config_path = hf_hub_download(
            repo_id=repo_id, filename=f"{config.name}/config.json"
        )
        model_path = hf_hub_download(
            repo_id=repo_id, filename=f"{config.name}/model.safetensors"
        )
    ...
    def metrics(self, x, x_hid, x_hat):
        ...
        metrics = dict()
        metrics["dead_fraction"] = (
            (self.inactive > self.config.dead_thresh).float().mean()
        )
    ...
    def push_to_hub(self, repo_id, tmp_dir="tmp"):
        ...
        HfApi().upload_folder(
            folder_path=tmp_dir, path_in_repo=self.config.name, repo_id=repo_id
        )
    ...
    def fit(self, model, train, validate, project: str | None = None):
        ...
        parameters = [
            ...
            dict(
                params=self.w_dec.parameters(), weight_decay=self.config.decoder_decay
            ),
        ]
        ...
        lr = self.config.lr
        optimizer = (
            ConstrainedAdam(parameters, [self.w_dec.weight], lr=lr)
            if self.config.normalize_decoder
            else Adam(parameters, lr=lr)
        )
        ...
        for idx, batch in pbar:
            ...
            if validate is not None and idx % self.config.val_steps == 0:
                ...
                metrics["val/recovered"] = 1 - (
                    (patched.item() - clean.item()) / (corrupt.item() - clean.item())
                )
            pbar.set_description(
                f"L1: {metrics['l1']:.4f}, NMSE: {metrics['nmse']:.4f}, added: {added:.4f}"
            )
