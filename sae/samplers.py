...
class BaseSampler:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.n_ctx, self.d_model, self.n_batches, self.in_batch = (
            n_ctx,
            d_model,
            n_batches,
            in_batch,
...
class ShuffleSampler(BaseSampler):
    ...
    def __iter__(self):
        ...
        while True:
            self.buffer = (
                self._buffer() if (self.buffer is None or step == 0) else self.buffer
            )
