...
def compute_self_and_cross_outliers(data, cross_factor, self_factor):
    ...
    cross = torch.sparse_coo_tensor(
        torch.stack([cidxs[0], *uidxs], dim=0), cross.values(), data.size()
    )

    selv = compute_outliers(data.diagonal(dim1=-2, dim2=-1), self_factor).coalesce()
    sidxs = selv.indices()
    selv = torch.sparse_coo_tensor(
        torch.stack([sidxs[0], sidxs[1], sidxs[1]], dim=0), selv.values(), data.size()
    )
