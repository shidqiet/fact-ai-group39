...
class Visualizer:
    def __init__(self, model, sae):
        path = hf_hub_download(
            repo_id=f"{model.config.repo}-scope", filename="top-activations.safetensors"
        )
        ...
        self.dataset = load_dataset(
            f"tdooms/{model.config.dataset}-16k", split="train"
        ).with_format("torch")
    ...
    def compute_max_activations(model, dataset=None, in_batch=16):
        dataset = (
            Visualizer.create_dataset_subset(model) if dataset is None else dataset
        )
        ...
        saes = [
            SAE.from_pretrained(
                f"{model.config.repo}-scope", point=point, expansion=8, k=30
            ).cuda()
            for point in points
        ]
        ...
        saes += [
            SAE.from_pretrained(
                f"{model.config.repo}-scope", point=points[-1], expansion=4, k=30
            ).cuda()
        ]
        ...
        HfApi().upload_file(
            path_or_fileobj="tmp.safetensors",
            path_in_repo="top-activations.safetensors",
            repo_id=f"{model.config.repo}-scope",
        )
    ...
    def color_line(line, colors, values, view):
        ...
        return "".join(
            [
                Visualizer.color_str(line[i], colors[i], values[i])
    ...
    def color_input_ids(self, input_ids, feature=0, view=range(-10, 20), dark=False):
        ...
        for line, color, value in zip(tokens, colors, values):
            print(
                f"{value.max().item():<4.1f}:  {Visualizer.color_line(line, color, value, view)}"
            )
