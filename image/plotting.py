...
def plot_explanation(model, sample: Float[Tensor, "w h"], eigenvalues=10):
    ...
    acts = (
        einsum(sample.flatten().cpu(), vecs, "inp, cls comp inp -> cls comp").pow(2)
        * vals
    )
    ...
    for i in range(3):
        ...
        fig.add_scatter(
            y=contrib[i, -1:].flip(0), mode="markers", **params, row=1, col=1
        )

        fig.add_scatter(
            y=contrib[i, : eigenvalues + 2], mode="lines", **params, row=2, col=1
        )
    ...
    for i in range(3):
        ...
        fig.add_heatmap(
            z=vecs[classes[i]][idxs[i, -1]].view(28, 28).flip(0),
            **params,
            row=1,
            col=i + 2,
        )
        fig.add_heatmap(
            z=vecs[classes[i]][idxs[i, 0]].view(28, 28).flip(0),
            **params,
            row=2,
            col=i + 2,
        )
    ...
    for i in range(2):
        ...
        fig.update_yaxes(
            visible=True, tickvals=tickvals, ticktext=ticktext, col=1, row=i + 1
        )
    ...
    fig.update_layout(
        width=800, height=320, margin=dict(l=0, r=0, b=0, t=20), template="plotly_white"
    )
