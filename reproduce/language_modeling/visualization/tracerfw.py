...
class Tracerfw:
    ...
    def __init__(
        ...
        repo = "Julianvn/facts-fw-small-scope"

        inp = dict(name="mlp-in", expansion=4, k=30) | inp
        out = dict(name="mlp-out", expansion=4, k=30) | out

        self.inp = SAE.from_pretrained(
            repo, point=(inp["name"], layer), expansion=inp["expansion"], k=inp["k"]
        ).to(device)
        self.out = SAE.from_pretrained(
            repo, point=(out["name"], layer), expansion=out["expansion"], k=inp["k"]
        ).to(device)

        self.out_latents = (
            self.out.w_enc.weight if use_encoder else self.out.w_dec.weight.T
        )
    ...
    def q(self, idx: int | slice, project=False):
        ...
        res = torch.einsum(
            ...
            model.w_r[layer],
            model.w_p[layer],
            self.out_latents[idx],
        )

        if project:
            res = torch.einsum(
                "il,jk,...ij->...lk", self.inp_latents, self.inp_latents, res
            )
