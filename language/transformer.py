...
class Attention(nn.Module):
    ...
    def forward(
        self, x: Float[Tensor, "batch seq d_model"], attn_mask=None
        ...
        q, k, v = rearrange(
            qkv,
            "batch seq (n n_head d_head) -> n batch n_head seq d_head",
            n=3,
            n_head=n_head,
        ).unbind(dim=0)
        q, k = self.rotary(q, k)
        ...
        if self.training:
            ...
            scores = einsum(
                q,
                k,
                "batch n_head seq_q d_head, batch n_head seq_k d_head -> batch n_head seq_q seq_k",
            )
            scores = scores / (torch.tensor(q.size(-1), device=x.device).sqrt())
            scores = scores.masked_fill(
                self.mask[:, :, : x.size(1), : x.size(1)].to(scores.device) == 0,
                -torch.inf,
            )
            ...
            z = einsum(
                pattern,
                v,
                "batch n_head seq_q seq_k, batch n_head seq_k d_head -> batch n_head seq_q d_head",
            )
...
class Transformer(PreTrainedModel):
    ...
    def forward(self, input_ids=None, labels=None, attention_mask=None, **kwargs):
        ...
        if labels is None:
            ...
            loss = self.criterion(
                shifted_logits.view(-1, logits.size(-1)), shifted_labels.view(-1)
            )
    ...
    def from_pretrained(cls, repo, device="cuda", **kwargs):
        ...
        return super(Transformer, Transformer).from_pretrained(
            repo, config=config, tokenizer=tokenizer, device_map=device, **kwargs
        )
    ...
    def tokenize(self, dataset):
        return self.tokenizer(
            dataset["text"], truncation=True, padding=True, max_length=self.config.n_ctx
        )
    ...
    def w_qkv(self):
        qkv = torch.stack(
            [self.transformer.h[i].attn.qkv.weight for i in range(self.config.n_layer)],
            dim=0,
        )
        return rearrange(
            qkv,
            "n_layer (n_proj n_head d_head) d_model -> n_proj n_layer n_head d_head d_model",
            n_proj=3,
            n_head=self.config.n_head,
        )
    ...
    def w_lr(self):
        lr = torch.stack(
            [self.transformer.h[i].mlp.w.weight for i in range(self.config.n_layer)],
            dim=0,
        )
        return rearrange(
            lr,
            "n_layer (n_proj d_hidden) d_model -> n_proj n_layer d_hidden d_model",
            n_proj=2,
        )
    ...
    def w_p(self):
        return torch.stack(
            [self.transformer.h[i].mlp.p.weight for i in range(self.config.n_layer)],
            dim=0,
        )
    ...
    def w_o(self):
        o = torch.stack(
            [self.transformer.h[i].attn.o.weight for i in range(self.config.n_layer)],
            dim=0,
        )
        return rearrange(
            o,
            "n_layer d_model (n_head d_head) -> n_layer n_head d_model d_head",
            n_head=self.config.n_head,
        )
    ...
    def generate(
        ...
        max_length = min(
            max_length or self.config.n_ctx, self.config.n_ctx - input_ids.size(-1) - 1
        )
