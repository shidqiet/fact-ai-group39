...
class Vocab:
    ...
    def make_labels(self, prompt: str):
        ...
        return (
            ["BOS"]
            + [f"{tok} {empty * cnt}" for tok, cnt in zip(tokens, counts)]
            + ["EOS"]
        )
    ...
    def __getitem__(
        self,
        key: str | int | List[int] | Int[Tensor, "indices"] | Tuple[int] | Tuple[str],
    ):
        ...
        if isinstance(key, str):
            ...
            return self.inv[key]
        elif (isinstance(key, list), isinstance(key, tuple)) and all(
